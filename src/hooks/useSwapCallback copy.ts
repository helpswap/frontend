...
import { JSBI, Percent, Router, SwapParameters, Trade, TradeType } from '@/helpswap/sdk'
...
export function useSwapCallback(
  ...
  return useMemo(() => {
    if (!trade || !library || !account || !chainId) {
      return { state: SwapCallbackState.INVALID, callback: null, error: t("common:missing-dependency") }
    }
    if (!recipient) {
              ...
              .catch((gasError) => {
                ...
                return contract.callStatic[methodName](...args, options)
                  .then((result) => {
                    ...
                    return { call, error: new Error(t("gas-error")) }
                  })
                  .catch((callError) => {
                    ...
                    switch (callError.reason) {
                      ...
                      case 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT':
                        errorMessage =
                          t("likely-slippage-error")
                        break
                      default:
                        errorMessage = <Trans i18nKey="swap:contract-error" components={[<Text as="span">{callError.reason}</Text>]} />
                    }
