...
const SlippageToleranceSettings = ({ translateString }: SlippageToleranceSettingsModalProps) => {
  ...
  useEffect(() => {
    try {
      ...
      if (!Number.isNaN(rawValue) && rawValue > 0 && rawValue < MAX_SLIPPAGE) {
        ...
        setError(translateString('Enter a valid slippage percentage'))
      }
    } catch {
      setError(translateString('Enter a valid slippage percentage'))
    }
  ...
  useEffect(() => {
    if (userSlippageTolerance < RISKY_SLIPPAGE_LOW) {
      setError(translateString('Your transaction may fail'))
    } else if (userSlippageTolerance > RISKY_SLIPPAGE_HIGH) {
      setError(translateString('Your transaction may be frontrun'))
    }
  ...
  return (
    <Box mb="16px">
      <Flex alignItems="center" mb="8px">
        <Text bold>{translateString('Slippage tolerance')}</Text>
        <QuestionHelper
          text={translateString('Your transaction will revert if the price changes unfavorably by more than this percentage.'
          )}
        />
      </Flex>
      <Options>
