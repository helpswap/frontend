import { Box, Text, HStack, Input, Img } from "@chakra-ui/react"
import { useEffect, useState } from "react"
import AssetView from "./AssetView"


const ItemTemplates = {
    profile: (props) => {
        return <AssetView {...props} />
    },
    asset: (props) => {
        return <AssetView {...props} />
    }
}

const SwapHelpersList = ({t, onSelected, itemTemplate, ...props}) => {
    const [helpers, setHelpers] = useState()

    const testIcon = "data:image/png;base64,"

    const [selectedIndex, setSelectedIndex] = useState(-1)
    useEffect(() => {
        setHelpers([
            {
                icon: testIcon,
                name: "Help/Jinminetics",
                symbol: "JMX",
                balance: 0,
            },
            {
                icon: testIcon,
                name: "Help/BashExchange",
                symbol: "BashEx",
                balance: 0
            }
        ])
    }, [])

    const onSelectedNative = (index) => {
        setSelectedIndex(index)
        if(onSelected) onSelected(index, helpers[index])
    }

    return (
        <Box w="100%">
            <Input w="100%" h={{base: "60px"}} placeholder={t("search-place-holder")} mb={{base: "2rem"}} />
            <Box w="100%">
                {
                    (helpers || []).map((value, index) => {
                        return (
                            <Box key={index} w="100%">
                                {ItemTemplates[itemTemplate || "asset"]({...value, index: index, selected: selectedIndex == index,
                                onSelected: () => {
                                    onSelectedNative(index)
                                }})}
                            </Box>
                        )
                    })
                }
            </Box>
        </Box>
    )

}

SwapHelpersList.ITEM_TEMPLATES = {
    profile: "profile",
    asset: "asset"
}

export default SwapHelpersList
